...
const logDiagnosticToConsole = (diagnostic) => {
    ...
    if (diagnostic.file) {
        ...
        filePosition += ` ${(0, path_1.relative)(process.cwd(), path)}`;
        if (position) {
    ...
    const now = new Date();
    const timestamp = (0, dateformat_1.default)(now, 'HH:MM:ss');
    ...
    if (typeof messageText !== 'string') {
        for (let i = 1; i < messageText.length; i += 1) {
            console.log(consoleColors[messageText[i].category]((0, indentString_1.default)(messageText[i].messageText, 4)));
        }
    }
};
