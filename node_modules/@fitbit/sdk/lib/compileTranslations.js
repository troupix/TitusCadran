...
function compileTranslations(defaultLanguage) {
    ...
    const transform = new stream_1.Transform({
        ...
        transform(file, _, next) {
            ...
            const languageTag = (0, path_1.basename)(file.basename, '.po');
            if (!(0, languageTag_1.validateLanguageTag)(languageTag)) {
                next(new plugin_error_1.default(PLUGIN_NAME, `Translation file ${file.basename} has a bad name. Translation files must be named ${(0, humanize_list_1.default)(Object.keys(ProjectConfiguration_1.Locales).map((tag) => tag + '.po'), { conjunction: 'or' })}.`, { fileName: file.relative }));
                return;
            }
            if (file.isBuffer()) {
                try {
                    ...
                    next(new plugin_error_1.default(PLUGIN_NAME, error, {
        ...
        flush(done) {
            ...
            for (const language of languageTable.languages) {
                ...
                try {
                    ...
                    done(new plugin_error_1.default(PLUGIN_NAME, error, {
                        fileName: file.relative,
                    }));
                    return;
                }
