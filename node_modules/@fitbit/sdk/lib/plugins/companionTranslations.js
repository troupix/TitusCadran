...
const loadPOFile = (0, util_1.promisify)(pofile_1.default.load);
...
function companionTranslations(globPattern, defaultLanguage) {
    return async () => {
        ...
        for (const filePath of await glob(globPattern)) {
            const languageTag = (0, path_1.basename)(filePath, '.po');
            if (!(0, languageTag_1.validateLanguageTag)(languageTag)) {
                throw new BuildError_1.default(`Translation file "${filePath}" has a bad name. Translation files must be named ${(0, humanize_list_1.default)(Object.keys(ProjectConfiguration_1.Locales).map((tag) => tag + '.po'), { conjunction: 'or' })}.`);
            }
        ...
        return (0, pluginutils_1.dataToEsm)(translations, { namedExports: false });
    };
}
