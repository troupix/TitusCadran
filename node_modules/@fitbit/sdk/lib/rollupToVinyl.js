...
function rollupToVinyl(inputOptions, outputOptions) {
    ...
    function emitAsset({ fileName, source }) {
        stream.push(new vinyl_1.default({
            ...
            path: (0, path_1.resolve)(process.cwd(), generatePath(fileName)),
        }));
    }
    function emitChunk({ code, fileName, map, isEntry }) {
        const chunkPath = generatePath(fileName);
        if (map) {
            map.mappings = JSON.parse((0, source_map_compactor_1.default)(map)).mappings;
            map.file = chunkPath;
        }
        stream.push(new vinyl_1.default({
            isEntryPoint: isEntry,
            contents: Buffer.from(code, 'utf8'),
            path: (0, path_1.resolve)(process.cwd(), chunkPath),
            sourceMap: map,
        }));
    }
    rollup
