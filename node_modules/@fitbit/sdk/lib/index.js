...
function generateBuildId() {
    return `0x0${(0, simple_random_1.default)({
...
function addErrorTarget(target, fn) {
    ...
    try {
        const buildStream = (0, eventsIntercept_1.default)(fn());
...
function loadProjectConfig({ hasNativeComponents = false, onDiagnostic = diagnostics_1.logDiagnosticToConsole, fileName = 'package.json', }) {
    try {
        const config = (0, ProjectConfiguration_1.normalizeProjectConfig)(JSON.parse(fs_1.default.readFileSync(fileName, 'utf-8')));
        const diagnostics = (0, ProjectConfiguration_1.validate)(config, { hasNativeComponents });
...
function buildComponent({ projectConfig, component, onDiagnostic = diagnostics_1.logDiagnosticToConsole, }) {
    ...
    const entryPoint = (0, findEntryPoint_1.default)(inputs, {
        onDiagnostic,
        component,
        notFoundIsFatal,
    });
    if (!entryPoint)
        return;
    return lazyObjectReadable(() => (0, multipipe_1.default)([
        (0, compile_1.default)({
            component,
            onDiagnostic,
            entryPoint,
        ...
        (0, gulpSetProperty_1.default)({
            componentType: component,
        }),
    ]));
}
exports.buildComponent = buildComponent;
function buildDeviceResources(projectConfig, { resourceFilterTag, defaultTXIOutputFormat }, onDiagnostic = diagnostics_1.logDiagnosticToConsole) {
    return (0, multipipe_1.default)([
        vinyl_fs_1.default.src('./resources/**', { base: '.' }),
        (0, filterResourceTag_1.default)(resourceFilterTag),
        (0, validateIcon_1.default)({ projectConfig, onDiagnostic }),
        (0, convertImageToTXI_1.default)({
            rgbaOutputFormat: defaultTXIOutputFormat
                ? defaultTXIOutputFormat
                : convertImageToTXI_1.TXIOutputFormat.RGBA6666,
        }),
        (0, gulpAssertFiles_1.default)([resources.svgMain, resources.svgWidgets]),
    ]);
}
...
function buildDeviceComponents({ projectConfig, buildId, onDiagnostic = diagnostics_1.logDiagnosticToConsole, }) {
    ...
    return multistream_1.default.obj([
        ...
        ...projectConfig.buildTargets.map((family) => lazyObjectReadable(() => {
            ...
            const sourceMap = (0, collectComponentSourceMaps_1.default)();
            return (0, multipipe_1.default)([
                (0, mergeStream_1.default)(...[
                    (0, multipipe_1.default)([
                    ...
                    (0, multipipe_1.default)([
                        ...
                        (0, compileTranslations_1.default)(projectConfig.defaultLanguage),
                    ]),
                ].filter(Boolean)),
                (0, componentManifest_1.makeDeviceManifest)({ projectConfig, buildId, targetDevice: family }),
                (0, zip_1.default)(bundleFilename),
                transformIf(maxDeviceBundleSize !== undefined, (0, validateFileSizes_1.default)({
                    onDiagnostic,
...
function buildCompanion({ projectConfig, buildId, onDiagnostic = diagnostics_1.logDiagnosticToConsole, }) {
    const sourceMaps = (0, collectComponentSourceMaps_1.default)();
    ...
    return lazyObjectReadable(() => (0, multipipe_1.default)([
        ...
        (0, componentManifest_1.makeCompanionManifest)({
            projectConfig,
            buildId,
            hasSettings: !!settings,
        }),
        (0, zip_1.default)('companion.zip'),
        (0, gulpSetProperty_1.default)({
...
function buildAppPackage({ projectConfig, buildId, existingDeviceComponents, onDiagnostic = diagnostics_1.logDiagnosticToConsole, }) {
    ...
    return (0, multipipe_1.default)([
        multistream_1.default.obj(components),
        (0, appPackageManifest_1.default)({
            projectConfig,
            buildId,
        }),
        (0, zip_1.default)('app.fba'),
    ]);
}
...
function buildProject({ nativeDeviceComponentPaths = [], onDiagnostic = diagnostics_1.logDiagnosticToConsole, }) {
    ...
    if (nativeDeviceComponentPaths.length > 0) {
        ({ buildId, existingDeviceComponents } = (0, nativeComponents_1.default)(projectConfig.appUUID, nativeDeviceComponentPaths));
    }
    else {
...
function build({ dest = vinyl_fs_1.default.dest('./build'), onDiagnostic = diagnostics_1.logDiagnosticToConsole, nativeDeviceComponentPaths, } = {}) {
    return new Promise((resolve, reject) => {
        ...
        try {
            (0, multipipe_1.default)([buildProject({ nativeDeviceComponentPaths, onDiagnostic }), dest], wrapBuildErrors);
        }
        catch (e) {
            wrapBuildErrors(e);
        }
