...
(function (Locales) {
    ...
    Locales["da-DK"] = "Danish (Denmark)";
    Locales["hi-IN"] = "Hindi (India)";
...
function getAllPermissionTypes(options) {
    ...
    return [
        ...
        semver_1.default.satisfies((0, sdkVersion_1.default)(), permission.sdkVersion) ||
        enableProposedAPI);
}
...
function validateAppType(config) {
    ...
    if (exports.VALID_APP_TYPES.indexOf(config.appType) === -1) {
        const appTypeNames = (0, humanize_list_1.default)(exports.VALID_APP_TYPES, { conjunction: 'or' });
...
function validateDefaultLanguage(config) {
    const diagnostics = new DiagnosticList_1.default();
    if (!(0, languageTag_1.validateLanguageTag)(config.defaultLanguage)) {
        diagnostics.pushFatalError(`Default language is an invalid language tag: ${config.defaultLanguage}. Must be ${(0, humanize_list_1.default)(languageTags, { conjunction: 'or' })}.`);
    }
    return diagnostics;
}
...
function validateStorageGroup(config) {
    const diagnostics = new DiagnosticList_1.default();
    const hasAccessAppClusterStoragePermission = (config.requestedPermissions || []).includes(Permission.ACCESS_APP_CLUSTER_STORAGE);
    if (hasAccessAppClusterStoragePermission) {
        if (config.appClusterID === undefined) {
            ...
            diagnostics.pushFatalError(`Only a maximum of ${MAX_NUM_CLUSTERS} App Cluster IDs may be declared`);
        }
        else {
